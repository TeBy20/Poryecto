...
class CajaController extends Controller
{
    ...
    public function buscarMovimientos(Request $request)
    {
        ...
        switch ($tipoMovimiento) {
            case 'ingresos':
                $resultados = Caja::whereBetween('fecha', [$fechaInicio, $fechaFin])
                    ->where('tipo', 'ingreso') // Filtra por tipo de ingreso
                    ->get();
                $montoTotal = $resultados->sum('monto');
                return view('caja.ingresos', compact('resultados', 'fechaInicio', 'fechaFin', 'montoTotal'));
            ...
            case 'egresos':
                $resultados = Caja::whereBetween('fecha', [$fechaInicio, $fechaFin])
                    ...
                    ->get();
                $montoTotal = $resultados->sum('monto');
                return view('caja.egresos', compact('resultados', 'fechaInicio', 'fechaFin', 'montoTotal'));
